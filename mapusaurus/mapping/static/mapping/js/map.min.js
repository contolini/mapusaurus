...
function initCalls(geoQueryType) {
    ...
    "selected" === gt ? $.when(getTractData(action, !1, !1)).done(function(data1) {
        init(data1);
    }) : "all_msa" === gt ? $.when(getTractData(action, getBoundParams(), "msa")).done(function(data1) {
        init(data1);
    }) : "all" === gt && $.when(getTractData(action, getBoundParams(), !1)).done(function(data1) {
        init(data1);
    });
}
...
function init(data1) {
    ...
    rawData = data1, createTractDataObj(), redrawCircles(dataStore.tracts, layerInfo.type), 
...
function drawCircle(geo, options) {
    ...
    circle.on("mouseover mousemove", function(e) {
        new L.Rrose({
            ...
            autoPan: !1,
            y_bound: 160
        }).setContent('<div class="bubble-header">Tract ' + circle.geoid + '</div><div class="lar-count"><span class="circle-hover-data">' + data.volume + '</span><span class="circle-hover-label">LAR</span></div><div class="hh-count"><span class="circle-hover-data">' + data.num_households + '</span><span class="circle-hover-label">Households</span></div></div>').setLatLng(e.latlng).openOn(map);
...
function createTractDataObj(callback) {
    dataStore.tracts = rawData, "function" == typeof callback && callback() && callback();
}
...
function toggleBranches(status) {
    status ? (drawBranches(), $("#branchKey").removeClass("hidden"), $("#lender-branches").addClass("green-highlight")) : (layers.Branches.clearLayers(), 
    $("#branchKey").addClass("hidden"), $("#lender-branches").removeClass("green-highlight")), 
    addParam("branches", status), $("#branchSelect").prop("checked", status);
}
...
function layerUpdate(layer) {
    ...
    $("#altScaleImgLabel").text(layerEval.displayName), $("#altScale").removeClass("hidden"), 
    ...
    layers.CountyLabels.bringToFront(), map.hasLayer(layers.MSALabels) && layers.MSALabels.bringToFront(), 
...
function getLayerType(layer) {
    ...
    switch (layer) {
      ...
      case "non_hisp_white_only_perc":
        layer = layers.PctWhite, type = "seq", keyPath = "/static/basestyle/img/key_pct-white.png", 
      ...
      case "owner_occupancy":
        layer = layers.OwnerOccupancy, type = "seq", keyPath = "/static/basestyle/img/key_own-occupancy.png", 
        displayName = "Owner Occupancy Rate";
        break;

      case "median_family_income":
        layer = layers.MedianIncome, type = "seq", keyPath = "/static/basestyle/img/key_med-fam-income.png", 
        displayName = "Median Family Income by Census Tract";
        break;

      case "median_value":
        layer = layers.MedianValue, type = "seq", keyPath = "/static/basestyle/img/key_med-hse-val.png", 
        displayName = "Median Value of O-O Housing";
        break;

      case "median_year":
        layer = layers.MedianYearBuilt, type = "seq", keyPath = "/static/basestyle/img/key_medyr-hse-built.png", 
        displayName = "Median Year Structure Built";
        break;

      default:
        layer = layers.PctMinority, type = "seq", keyPath = !1, displayName = "Percentage Minority";
    }
    return {
        type: type,
        layer: layer,
...
function generateTooltips(selector, offset) {
    offset || (offset = [ 1, 0 ]), selector ? $(selector + " .tooltipsy").tooltipsy({
...
$(document).ready(function() {
    ...
    $(".tabs").show(), $(window).resize(function() {
        window.location.href.indexOf("print") < 0 && setMapHeight();
    ...
    var keyHide = $(".hide-key"), keyShow = $(".show-key"), keyContents = $(".key-contents"), minorityKeyContents = $(".minority-key");
    keyHide.on("click", function(e) {
        keyShow.removeClass("hidden"), keyContents.addClass("hidden"), minorityKeyContents.addClass("hidden"), 
        keyHide.addClass("hidden");
    }), keyShow.on("click", function(e) {
        keyHide.removeClass("hidden"), keyContents.removeClass("hidden"), minorityKeyContents.removeClass("hidden"), 
    ...
    }), getPeerLinks(), initCalls(geoQueryType), $(".leaflet-control-layers-toggle").css({
...
var rawGeo, rawLar, rawMinority, rawData, selectedYear = selectedYear || 2014, showingLoading = !1, larVolume = [], pctMinority = [], lastRequest, dataStore = {};

dataStore.tracts = {}, $.ajaxSetup({
    beforeSend: function(jqXHR) {
        lastRequest && lastRequest.abort(), lastRequest = jqXHR, $("#loading").removeClass("hidden");
    },
